...
class _Kettle():
    ...
    def __init__(self, args, batch_size, augmentations, mixing_method=dict(type=None, strength=0.0),
                 ...
                 custom_training_data=None, train_indices=None, source_class=0, target_class=1):
        ...
        # D1 
        if custom_training_data is None:
            self.trainset, self.validset = construct_datasets(self.args.dataset, self.args.data_path, NORMALIZE, train_indices)

        # D2
        # TODO : custom traing dataset, add witch brew poison to training set √
        else:
            poison_delta,poison_indices=custom_training_data
            self.trainset, self.validset = construct_datasets(self.args.dataset, self.args.data_path, NORMALIZE, train_indices)
            if args.dataset=='ImageNette':
                self.trainset.poison_indices=poison_indices
                self.trainset.poison_delta=poison_delta
            elif args.dataset=='CIFAR10':
                self.trainset.data[poison_indices]=self.trainset.data[poison_indices]+poison_delta
            elif args.dataset=='SVHN':
                poison_delta=poison_delta.transpose(0,3,1,2)
                self.trainset.data[poison_indices]=self.trainset.data[poison_indices]+poison_delta
            elif args.dataset=='STL10':
                poison_delta=poison_delta.transpose(0,3,1,2)
                self.trainset.data[poison_indices]=self.trainset.data[poison_indices]+poison_delta
            #self.trainset.data = self.trainset.data[custom_training_data]
            #self.trainset.targets = np.array(self.trainset.targets)[custom_training_data].tolist()
        ...
        self.prepare_experiment(source_class,target_class)
    ...
    def print_status(self):
        class_names = self.trainset.classes + ['None']*90
    ...
    def prepare_diff_data_augmentations(self, normalize=True):
        ...
        if self.augmentations is not None or self.args.paugment:
            if 'CIFAR' in self.args.dataset:
            ...
            elif 'STL10' in self.args.dataset:
                params = dict(source_size=96, target_size=96, shift=96//4, fliplr=True)
            elif 'SVHN' in self.args.dataset:
                params = dict(source_size=32, target_size=32, shift=32//4, fliplr=True)
                ...
                params = dict(source_size=128, target_size=128, shift=128 // 4, fliplr=True)

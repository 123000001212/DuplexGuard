...
def get_model(model_name, dataset_name, pretrained=False):
    ...
    if 'CIFAR' in dataset_name or 'MNIST' in dataset_name or 'STL10' in dataset_name or 'SVHN' in dataset_name or 'GTSRB' in dataset_name:
        if pretrained:
            raise ValueError('Loading pretrained models is only supported for ImageNet.')
        in_channels = 1 if dataset_name == 'MNIST' else 3
        if dataset_name in ['CIFAR10', 'MNIST','STL10','SVHN']:
            num_classes = 10 
        ...
        if 'efficientnet-b' in model_name.lower():
            ...
            try:
                model = getattr(torchvision.models, model_name.lower())(pretrained=pretrained, num_classes=num_classes, **extra_args)

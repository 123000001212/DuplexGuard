...
def options():
    ...
    parser.add_argument('--dataset', default='CIFAR10', type=str, choices=['CIFAR10', 'CIFAR100', 'ImageNet', 'ImageNet1k', 'MNIST', 'TinyImageNet','GTSRB', 'STL10', 'SVHN'])
    ...
    parser.add_argument('--threatmodel', default='untargeted', type=str, choices=['single-class', 'third-party', 'random-subset','untargeted'])
    ...
    parser.add_argument('--patch_size', default=12, type=int, help='Size of patch to be added to source images')
    # Files and folders
    ...
    parser.add_argument('--load_patch', default='./SleeperAgent/patches/trigger_10.png', type=str, help='Path to load image for patch attack')
    ...
    parser.add_argument('--skip_clean_training', default=False, help='Skip clean training. This is only suggested for attacks that do not depend on a clean model.')
    ...
    parser.add_argument('--dryrun', default=False, help='This command runs every loop only a single time.')
    ...
    parser.add_argument('--sources_train', default=4000, type=int, help='Number of sources for training')
    parser.add_argument('--source_gradient_batch', default=1000, type=int, help='Batch size for sources train gradient computing')
...
def options_imagenet():
    ...
    ###########################################################################
    # Central:
    parser.add_argument('--net', default='ResNet34', type=lambda s: [str(item) for item in s.split(',')])
    parser.add_argument('--dataset', default='ImageNette', type=str, choices=['CIFAR10', 'CIFAR100', 'ImageNet', 'ImageNet1k', 'MNIST', 'TinyImageNet','ImageNette'])
    parser.add_argument('--recipe', default='gradient-matching', type=str, choices=['gradient-matching', 'gradient-matching-private', 'gradient-matching-mt',
                                                                                    'watermark', 'poison-frogs', 'metapoison', 'hidden-trigger',
                                                                                    'metapoison-v2', 'metapoison-v3', 'bullseye', 'patch',
                                                                                    'gradient-matching-hidden', 'convex-polytope'])
    ...
    parser.add_argument('--budget', default=0.05, type=float, help='Fraction of training data that is poisoned')
    parser.add_argument('--sources', default=300, type=int, help='Number of sources')
    parser.add_argument('--patch_size', default=45, type=int, help='Size of patch to be added to source images')
    # Files and folders
    parser.add_argument('--name', default='', type=str, help='Name tag for the result table and possibly for export folders.')
    parser.add_argument('--table_path', default='tables/', type=str)
    parser.add_argument('--poison_path', default='poisons/', type=str)
    parser.add_argument('--data_path', default='/home/data/imagenette2-160', type=str)
    parser.add_argument('--modelsave_path', default='./models/', type=str)
    ###########################################################################
    ...
    parser.add_argument('--optimization', default='conservative', type=str, help='Optimization Strategy')
    # Strategy overrides:
    parser.add_argument('--epochs', default=80, type=int, help='Override default epochs of --optimization strategy')
    ...
    parser.add_argument('--source_gradient_batch', default=250, type=int, help='Batch size for sources train gradient computing')
    parser.add_argument('--val_max_epoch', default=80, type=int, help='Train only up to this epoch for final validation.')
    parser.add_argument('--retrain_max_epoch', default=80, type=int, help='Train only up to this epoch for retraining during crafting.')
    parser.add_argument('--retrain_scenario', default='from-scratch', type=str, choices=['from-scratch', 'finetuning'])
    parser.add_argument('--load_feature_repr', action='store_true', help='Load feature representation of the model trained on clean data')
    parser.add_argument('--random_patch', action='store_true', default=True, help='Adding patch to sources on a random location')
    parser.add_argument('--retrain_iter', default=50, type=int, help='Start retraining every <retrain_iter> iterations')
